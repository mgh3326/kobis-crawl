...
class WeeklyBoxOfficeCrawler:
    load_dotenv()
    API_KEY = os.getenv("KOBIS_API_KEY")
    BOX_OFFICE_BASE_URL = f"http://kobis.or.kr/kobisopenapi/webservice/rest/boxoffice/searchWeeklyBoxOfficeList.json?key={API_KEY}"
    ...
    def run(self):
        ...
        if response is not None and response.status_code == 200:
            ...
            box_office_results = data["boxOfficeResult"]["weeklyBoxOfficeList"]
    ...
    def _get_response_from_api(self):
        ...
        try:
            response = requests.get(
                WeeklyBoxOfficeCrawler.BOX_OFFICE_BASE_URL, params=params
            )
